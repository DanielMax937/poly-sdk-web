...
export default function FuturesTraderPage() {
    ...
    const searchMarkets = useCallback(async (keyword: string) => {
        ...
        try {
            const res = await fetch(`/api/base/search?q=${encodeURIComponent(keyword)}&limit=15&checkInsider=true&insiderTimeout=8000`);
    ...
    const startMonitoring = useCallback(async (market: Market) => {
        ...
        const takeSnapshot = async () => {
            try {
                const res = await fetch(`/api/base/orderbook?conditionId=${market.conditionId}`);
                const data = await res.json();
