...
与 Cursor **local-service** skill 约定一致：默认在 **3020** 端口后台启动 Next 开发服务（避免占用 3000），健康检查为 `/api/health`。

```bash
./start-bg.sh    # PORT=3020，日志 poly-sdk-web.log
./stop-bg.sh
# 自定义端口：PORT=3030 ./start-bg.sh  （stop 需相同 PORT）
```
